...
module.exports = {
    ...
    .addSubcommand(command => command.setName('start').setDescription('Start a giveaway').addStringOption(option => option.setName('duration').setDescription(`Duration of the giveaway (ie. 1m, 1h, 1d, etc)`).setRequired(true)).addIntegerOption(option => option.setName('winners').setDescription('How many people can win this giveaway').setRequired(true)).addStringOption(option => option.setName('prize').setDescription('The prize of this giveaway').setRequired(true)).addChannelOption(option => option.setName('channel').setDescription('The channel the giveaway will send to')).addStringOption(option => option.setName('content').setDescription('Content of the giveaway embed')))
    .addSubcommand(command => command.setName(`edit`).setDescription(`Edit a giveaway.`).addStringOption(option => option.setName('message-id').setDescription('The ID of the giveaway to edit').setRequired(true)).addStringOption(option => option.setName('time').setDescription('The amount of time in ms to add to the giveaway').setRequired(true)).addIntegerOption(option => option.setName('winners').setDescription('The new amount of winners for this giveaway').setRequired(true)).addStringOption(option => option.setName('prize').setDescription('The new prize for this giveaway').setRequired(true)))
    .addSubcommand(command => command.setName('end').setDescription(`End a giveaway.`).addStringOption(option => option.setName('message-id').setDescription('The ID of the giveaway to end').setRequired(true)))
    .addSubcommand(command => command.setName(`reroll`).setDescription(`Reroll a giveaway.`).addStringOption(option => option.setName('message-id').setDescription('The ID of the giveaway to reroll').setRequired(true))),
    async execute(interaction, client) {
        ...
        switch (sub) {
            ...
            interaction.editReply({ content: `<:check:1266815137587920937> Started giveaway`, ephemeral: true })
           ...
           await interaction.reply({ content: `<:bubble:1266834037931118623> Editing giveaway...`, ephemeral: true});
            ...
            client.giveawayManager.reroll(messageId2).then(() => {
                interaction.editReply({ content: `<:check:1266815137587920937> Rerolles giveaway`});
            })
            .catch((err) => {
